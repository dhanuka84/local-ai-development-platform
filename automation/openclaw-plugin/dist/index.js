    ...
    tools: (tool) => [
        tool({
            ...
            factory: ({ api, config, toolContext }) => ({
                ...
                description: "Create one managed Task Flow and one idempotent authoritative workflow.",
                parameters: startParameters,
                execute: async (_toolCallId, params, signal) => jsonResult(await new WorkflowController(api, toolContext, config).start(params, signal)),
            }),
        }),
        tool({
        ...
        tool({
            ...
            factory: ({ api, config, toolContext }) => ({
                ...
                description: "Apply an authorized optimistic PostgreSQL transition and then mirror it to Task Flow.",
                parameters: transitionParameters,
                execute: async (_toolCallId, params, signal) => jsonResult(await new WorkflowController(api, toolContext, config).transition(params, signal)),
            }),
        }),
        tool({
            ...
            factory: ({ api, config, toolContext }) => ({
                ...
                description: "Persist sticky cancellation intent before cancelling OpenClaw child tasks.",
                parameters: cancelParameters,
                execute: async (_toolCallId, params, signal) => jsonResult(await new WorkflowController(api, toolContext, config).cancel(params, signal)),
            }),
        }),
        tool({
            ...
            factory: ({ api, config, toolContext }) => ({
                ...
                description: "Queue an atomic task. Auto mode is the default; only the FIFO head activates and runs RAG routing.",
                parameters: taskQueueParameters,
                execute: async (_toolCallId, params, signal) => jsonResult(await new WorkflowController(api, toolContext, config).queueTask(params, signal)),
            }),
        }),
        tool({
        ...
        tool({
            ...
            factory: ({ api, config, toolContext }) => ({
                ...
                description: "Record an optimistic provider-gated task checkpoint event and immutable evidence.",
                parameters: taskTransitionParameters,
                execute: async (_toolCallId, params, signal) => jsonResult(await new WorkflowController(api, toolContext, config).transitionTask(params, signal)),
            }),
        }),
    ],
});
