...
integration-test-fresh: ## Run PostgreSQL migrations and adapter tests in newly pulled disposable images
	@set -eu; \
	integration_project="hybrid-ai-integration-$$$$"; \
	cleanup() { docker compose -p "$$integration_project" -f deploy/compose/compose.integration.yaml down --volumes --remove-orphans; }; \
	trap cleanup EXIT INT TERM; \
	docker compose -p "$$integration_project" -f deploy/compose/compose.integration.yaml pull; \
	docker compose -p "$$integration_project" -f deploy/compose/compose.integration.yaml up \
		--abort-on-container-exit --exit-code-from integration-test --force-recreate
