...
func TestValidationBindsAuthenticatedHumanAndVerifiesSourceBytes(t *testing.T) {
	...
	if _, err := svc.RecordManualValidation(identity.WithPrincipal(t.Context(), workload), input); !errors.Is(err, ErrForbidden) {
...
func TestDecisionRequiresEvidenceAndDerivesActor(t *testing.T) {
	...
	workload := soloDeveloper()
	workload.Human = false
	if _, err := svc.DecideKnowledge(identity.WithPrincipal(t.Context(), workload), input); !errors.Is(err, ErrForbidden) {
	...
	if _, err := svc.DecideKnowledge(identity.WithPrincipal(t.Context(), wrongProject), input); !errors.Is(err, ErrForbidden) {
